...
local function request_in_motion(request)
	...
	local group = request.group
	if group and group.valid then
		s1 = (request.group.command ~= nil)
	end

	group = request.group_boss
	if group and group.valid then
		s2 = (request.group_boss.command ~= nil)
	end
...
-- request_attack - Puts the unit group into motion.
local function request_attack(group, p_force)
	...
	local position = AiTargets.poll(p_force)
	...
	AiStrikes.initiate(group, p_force, position)
end
...
local function _form_groups(b_force, p_force)
	...
	for i = 1, global.request_max_groups do
		...
		if step_group_formation(request) then
			request_attack(request.group, p_force)
			request_attack(request.group_boss, p_force)
		end
