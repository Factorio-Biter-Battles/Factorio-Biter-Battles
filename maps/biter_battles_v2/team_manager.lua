...
local function isReferee(player)
  if global.active_special_games['captain_mode'] and global.special_games_variables['captain_mode']['refereeName'] ==
      player.name then
    return true
  else
    return false
...
local function team_manager_gui_click(event)
  ...
  if name == 'team_manager_activate_tournament' then
    ...
    if global.active_special_games['captain_mode'] == true and
        global.special_games_variables['captain_mode']['prepaPhase'] == true then
      player.print('You cant disable tournament mode during prepa phase of captain event !', { r = 175, g = 0, b = 0 })
      return
    end
    if global.tournament_mode then
