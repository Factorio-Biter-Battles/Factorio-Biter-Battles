...
function Public.do_raw_feed(flask_amount, food, biter_force_name, tier)
    ...
    if Quality.enabled() then
        food_value = food_value * Quality.TIERS[tier].multiplier
    end
...
function Public.feed_biters_from_inventory(player, food)
    ...
    if Quality.enabled() then
        req['quality'] = Quality.TIERS[Quality.selected_by(player)].name
    end
    ...
    local rm = { name = food, count = flask_amount }
    if Quality.enabled() then
        rm['quality'] = req['quality']
    end
    i.remove(rm)

    print_feeding_msg(player, food, flask_amount, req['quality'])
    ...
    if Quality.enabled() then
        Public.do_raw_feed(flask_amount, food, biter_force_name, Quality.selected_by(player))
    else
        Public.do_raw_feed(flask_amount, food, biter_force_name, nil)
    end
...
local function spend_food(player, inventory, enemy_force, biter_force, food, tier, msg)
    ...
    if tier then
        req['quality'] = Quality.TIERS[tier].name
    end

    local flask_amount = inventory.get_item_count(req)
    if flask_amount == 0 then
        return
    end
    ...
    entry = player.name .. ' fed ' .. flask_amount .. ' flasks of ' .. food_values[food].name
    if Quality.enabled() then
    ...
    Public.do_raw_feed(flask_amount, food, biter_force, tier)
    Public.add_feeding_stats(
        player,
        player.force.name,
        food,
...
function Public.feed_biters_mixed_from_inventory(player, button)
    ...
    for k, v in pairs(food) do
        if Quality.enabled() then
            for tier = 1, #Quality.TIERS do
                spend_food(player, i, enemy_force_name, biter_force_name, v, tier, message)
            end
        else
            spend_food(player, i, enemy_force_name, biter_force_name, v, nil, message)
        end
    end
