...
local function is_horizontal_border_river(pos)
	...
	-- from pre-computed table and then smooth it out by multiplying with 0.25.
	-- Position X coordinate is used to determinate which offset is selected.
	local offset = river.offset[pos.x % river.size]
	offset = offset * 0.25
		...
		for _ = 0, 31, 1 do
			...
			if is_horizontal_border_river(pos) and not is_within_spawn_island(pos) then
				...
				if math_random(1, 64) == 1 then
					surface.create_entity({name = "fish", position = unref_pos})
				end
			else
				tiles[i] = nil
			end
