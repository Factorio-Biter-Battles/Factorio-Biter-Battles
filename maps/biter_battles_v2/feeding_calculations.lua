...
function Public.calc_send_command(
    ...
    local chances_str = ''
    for i = 2, tiers do
        if q_values[i] ~= 0 then
            local diff = Quality.chance_difference(q_values[i], i, biter_force_name)
            chances_str = chances_str .. string.format('\n %s: +%.2f%%', Quality.TIERS[i].name, 100 * diff)
        end
    end
