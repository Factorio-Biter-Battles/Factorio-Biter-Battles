...
Factorio replaces `math.pow`, `math.log`, and `math.log10` with custom deterministic implementations (not from the system's libm). This ensures identical floating-point results across all platforms and CPU architectures, which is critical for multiplayer determinism.
...
- **Non-deterministic iteration order.** `pairs()` over the hash part of a table has no guaranteed order in standard Lua 5.2. Write code as if the order is arbitrary. Note: the Factorio Lua runtime's internal table implementation happens to iterate the hash part in insertion order (doubly-linked node list), so `pairs()` may appear stable in-game. Do not rely on this -- the unit test suite runs on standard Lua where the guarantee does not hold, and any code depending on insertion-ordered `pairs()` will fail there.
...
**Randomness is deterministic.** `math.random` is seeded identically on all clients and produces the same sequence everywhere. There is no per-client entropy. For anything where unpredictability matters (e.g., loot rolls that players might try to game), prefer re-seeding with a value derived from high-frequency, hard-to-predict inputs such as accumulated player movement positions or action timestamps. This does not make the outcome cryptographically secret -- a cheat running in memory can still observe and predict the state -- but it raises the practical cost of exploitation significantly compared to a fixed or tick-based seed.
