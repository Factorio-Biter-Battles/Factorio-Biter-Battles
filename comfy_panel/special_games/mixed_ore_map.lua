...
local function generate_mixed_ore_map(type, size)
    if type then
        ...
        game.print(
            'Special game Mixed ore map is being scheduled. The special game will start after restarting the map!',
            { color = Color.warning }
        )
    end
end
...
local Public = {
    name = {
        type = 'label',
        caption = 'Mixed ore map',
        tooltip = 'Covers the entire map with mixed ore. Takes effect after map restart',
    },
    config = {
        ...
        [2] = {
            name = 'type1',
            type = 'drop-down',
            items = { 'Mixed ore', 'Checkerboard', 'Vertical lines', 'Patches', 'Dots' },
        },
        ...
        [4] = {
            ...
            numeric = true,
            width = 40,
            tooltip = 'Live empty for default'
