...
local function clear_gui_captain_mode()
  for _, player in pairs(game.players) do
    local top_guis =
        { 'captain_manager_toggle_button', 'captain_referee_toggle_button', 'captain_player_toggle_button' }
    local center_guis = {
    ...
    for _, gui_name in pairs(top_guis) do
      local top_button = Gui.get_top_element(player, gui_name)
      if top_button then
        top_button.destroy()
      end
    end
    for _, gui in pairs(center_guis) do
      ...
      if player_gui.center[gui] then
        player_gui.center[gui].destroy()
      end
      if player_gui.screen[gui] then
        player_gui.screen[gui].destroy()
      end
    end
  end
end
...
local function pickPlayerGenerator(player, tableBeingLooped, frameName, questionText, button1Text, button1Name)
  if player.gui.center[frameName] then
    player.gui.center[frameName].destroy()
    return
  end
  local frame = player.gui.center
                    .add { type = 'frame', caption = questionText, name = frameName, direction = 'vertical' }
    ...
    for _, pl in pairs(tableBeingLooped) do
      if button1Text ~= nil then
        ...
        if global.total_time_online_players[playerIterated.name] then
          playtimePlayer = Player_list.get_formatted_playtime_from_ticks(
...
local function poll_alternate_picking(player)
  pickPlayerGenerator(player, global.special_games_variables['captain_mode']['listPlayers'],
                      'captain_poll_alternate_pick_choice_frame', 'Who do you want to pick ?', 'Magical1@StringHere',
...
local function generateGenericRenderingCaptain()
  local y = -14
  renderText('captainLineOne', 'Special event rule only : ', { -65, y }, { 1, 1, 1, 1 }, 3, 'heading-1')
  y = y + 2
  renderText('captainLineTwo',
  ...
  y = y + 4
  renderText('captainLineThree', 'Notes: ', { -65, y }, { 1, 1, 1, 1 }, 2.5, 'heading-1')
  y = y + 2
  renderText('captainLineFour', '-Chat of spectator can only be seen by spectators for players', { -65, y },
             { 1, 1, 1, 1 }, 2.5, 'heading-1')
  y = y + 2
  ...
  renderText('captainLineEight', '-If you are not playing, you can listen to any team, but your mic must be off',
             { -65, y }, { 1, 1, 1, 1 }, 2.5, 'heading-1')
  y = y + 2
  renderText('captainLineNine', '-No sign up required, anyone can play the event!', { -65, y }, { 1, 1, 1, 1 }, 2.5,
             'heading-1')
  y = y + 2
end
    ...
    for _, playerName in ipairs(special['listPlayers']) do
      ...
      if global.chosen_team[playerName] == nil and player.tag == playerChecked.tag and player.force.name == 'spectator' then -- only pick player without a team within the same group
        if amountPlayersSwitchedForGroup < special['groupLimit'] - 1 then
          ...
          game.print(playerName .. ' was not picked automatically with group system, as the group limit was reached',
                     Color.red)
        end
      end
    end
...
local function is_player_in_group_system(playerName)
  ...
  if global.special_games_variables['captain_mode']['captainGroupAllowed'] then
    local playerChecked = cpt_get_player(playerName)
    if playerChecked and playerChecked.tag ~= '' and
        startswith(playerChecked.tag, ComfyPanelGroup.COMFY_PANEL_CAPTAINS_GROUP_PLAYER_TAG_PREFIX) then
      return true
    end
  end
  return false
...
local function allow_vote()
  ...
  game.print('[font=default-large-bold]Difficulty voting is opened until the referee starts the picking phase ![/font]',
             Color.cyan)
end
...
local function generate_captain_mode(refereeName, autoTrust, captainKick, specialEnabled)
  ...
  if referee == nil then
    game.print('Event captain aborted, referee is not a player connected. Provided referee name was: ' ..
                   special['refereeName'])
    global.special_games_variables['captain_mode'] = nil
    global.active_special_games['captain_mode'] = false
    return
  ...
  if not check_if_enough_playtime_to_play(referee) then
    game.print('Referee does not seem to have enough playtime (which is odd), so disabling min playtime requirement',
               Color.red)
  ...
  for _, pl in pairs(game.connected_players) do
    ...
    Public.draw_captain_player_gui(pl)
		Public.draw_captain_tournament_button(pl)
  ...
  renderText('captainLineEleven', 'team xx vs team yy. Referee: ' .. refereeName .. '. Teams on VC', { 0, 10 },
             Color.captain_versus_float, 1.5, 'heading-1')
...
local function generate_vs_text_rendering()
  if global.active_special_games and global.special_games_variables['rendering'] and
      global.special_games_variables['rendering']['captainLineVersus'] then
  ...
  local text = string.format('Team %s (North) vs (South) Team %s. Referee: %s. Teams on Voice Chat',
                             special['captainList'][1], special['captainList'][2], special['refereeName'])
...
local function start_captain_event()
  ...
  global.special_games_variables['captain_mode']['stats']['NorthInitialCaptain'] =
      global.special_games_variables['captain_mode']['captainList'][1]
  global.special_games_variables['captain_mode']['stats']['SouthInitialCaptain'] =
      global.special_games_variables['captain_mode']['captainList'][2]
  global.special_games_variables['captain_mode']['stats']['InitialReferee'] =
      global.special_games_variables['captain_mode']['refereeName']
  ...
  rendering.clear()
  renderText('captainLineSeventeen', 'Special Captain\'s tournament mode enabled', { 0, -16 }, { 1, 0, 0, 1 }, 5,
             'heading-1')
  generate_vs_text_rendering()
  generateGenericRenderingCaptain()
  renderText('captainLineEighteen', 'Want to play? Click \'Join Info\' button at top of screen!', { 0, -9 },
             { 1, 1, 1, 1 }, 3, 'heading-1')
...
local countdown_captain_start_token = Token.register(function()
  if global.special_games_variables['captain_mode']['countdown'] > 0 then
    ...
    global.special_games_variables['captain_mode']['countdown'] =
        global.special_games_variables['captain_mode']['countdown'] - 1
  else
    for _, player in pairs(game.connected_players) do
      if player.gui.center['bb_captain_countdown'] then
        player.gui.center['bb_captain_countdown'].destroy()
...
local function captain_log_start_time_player(player)
  if global.special_games_variables['captain_mode'] ~= nil and
      (player.force.name == 'south' or player.force.name == 'north') and
      not global.special_games_variables['captain_mode']['prepaPhase'] then
...
function Public.draw_captain_manager_gui(player)
  ...
  local frame = ClosableFrame.create_main_closable_frame(player, 'captain_manager_gui', 'Cpt Captain')
  ...
  frame.add({ type = 'button', name = 'captain_toggle_throw_science' })
...
function Public.update_captain_manager_gui(player)
  ...
  if special['prepaPhase'] and not isStringInTable(special['listTeamReadyToPlay'], force_name) then
    ...
    if game.ticks_played < global.difficulty_votes_timeout then
      ...
      frame.diff_vote_duration.caption = string.format(
  ...
  frame.trusted_to_throw_list_label.caption = 'List of players trusted to throw : ' ..
                                                  table.concat(tablePlayerListThrowAllowed, ' | ')
  local team_players = {}
...
function Public.draw_captain_tournament_button(player)
    ...
    tooltip = '[font=default-bold]Tournament menu[/font] - Toggle the tournament tabs',
    index = Gui.get_top_index(player),
  })
end
...
function Public.draw_captain_player_gui(player)
  ...
  local special = global.special_games_variables['captain_mode']
  local frame = ClosableFrame.create_draggable_frame(player, 'captain_player_gui', 'Join Info')
  ...
  local want_to_be_captain_row = frame.add({
    type = 'table',
    name = 'captain_player_want_to_be_captain_row',
    column_count = 2,
  })
  b = want_to_be_captain_row.add({
    type = 'button',
  ...
  l = player_info_flow.add({
    type = 'label',
    name = 'captain_player_info_label',
    caption = 'Enter any info you want the captains to see when picking players,\ni.e. \'I will be on discord. I can threatfarm. I can build lots of power.\'',
  })
  l.style.single_line = false
  local textbox = player_info_flow.add({
    type = 'textfield',
    name = 'captain_player_info',
    text = special['player_info'][player.name] or '',
    tooltip = 'Enter any info you want the captains to see when picking players.',
  })
...
function Public.update_captain_player_gui(player)
  ...
  if special['prepaPhase'] then
    ...
    if not special['initialPickingPhaseStarted'] then
      ...
      cpt_volunteers.caption = 'Captain volunteers (' .. #special['captainList'] .. '): ' ..
      ...
      rem.visible = true
      rem.caption = 'Players remaining to be picked (' .. #special['listPlayers'] .. '): ' ..
  ...
  if global.chosen_team[player.name] then
    table.insert(status_strings, 'On team ' .. global.chosen_team[player.name] .. ': ' ..
  ...
  if not global.chosen_team[player.name] and not special['pickingPhase'] and not special['kickedPlayers'][player.name] then
    ...
    if special['prepaPhase'] and not special['initialPickingPhaseStarted'] then
      if special['captainGroupAllowed'] then
        table.insert(status_strings, string.format('Groups of players: ENABLED, group name must start with "%s"',
  ...
  if #sorted_players > 0 then
    ...
    for _, player_name in ipairs(sorted_players) do
      ...
      tab.add({
        type = 'label',
        caption = string.format('%s (%d%%)', Functions.format_ticks_as_time(info.playtime), 100 * playtime_frac),
      })
...
function Public.draw_captain_referee_gui(player)
  ...
  local frame = ClosableFrame.create_main_closable_frame(player, 'captain_referee_gui', 'Cpt Referee')
  frame.style.maximal_width = 800
...
function Public.draw_captain_tournament_gui(player)
  ...
  local main_frame = Gui.get_left_element(player, 'captain_tournament_gui')
	...
	local main_frame = Gui.add_left_element(player, { type = 'frame', name = 'captain_tournament_gui', direction = 'vertical'})
  ...
  local sp = inner_frame
	...
	local function add_shortcut_selection_row(parent, p)
		local frame = parent.add { type = 'frame', style = 'shortcut_selection_row', direction = 'horizontal', name = p.name }
		gui_style(frame, { height = 36, left_padding = 4, right_padding = 4, use_header_filler = false, horizontally_stretchable = true, vertically_stretchable = false })

		local icon = frame.add { type = 'sprite-button', style = 'transparent_slot', sprite = p.sprite, hovered_sprite = p.hovered_sprite }
	...
	for _, params in pairs({
		{ name = 'player_gui', sprite = 'entity/big-biter', caption = 'Join tournament', tooltip = 'Toggle the join window' },
		{ name = 'referee_gui', sprite = 'achievement/lazy-bastard', caption = 'Referee', tooltip = 'Toggle the referee window' },
		{ name = 'manager_gui', sprite = 'utility/force_editor_icon', caption = 'Tournament Manager', tooltip = 'Toggle the manager window' },
		{ name = 'organization_gui', sprite = 'utility/slot_icon_robot_material', caption = 'Team Organization', tooltip = 'Toggle your team organization window' },
	}) do add_shortcut_selection_row(sp, params) end
...
Gui.on_click('tournament_subheader_toggle', function(event)
...
local function insertPlayerByPlaytime(playerName)
  ...
  if isStringInTable(listPlayers, playerName) then
    return
  end
...
local function end_of_picking_phase()
  ...
  if not special['initialPickingPhaseFinished'] then
    ...
    if special['captainGroupAllowed'] then
      game.print('[font=default-large-bold]Initial Picking Phase done - group picking is now disabled[/font]',
                 Color.cyan)
    end
  end
  ...
  if special['prepaPhase'] then
    ...
    for _, captain_name in pairs(global.special_games_variables['captain_mode']['captainList']) do
      ...
      captain.print('As a captain, you can handle your team by clicking on \'Cpt Captain\' button top of screen',
                    { r = 1, g = 1, b = 0 })
...
local function start_picking_phase()
  ...
  if special['prepaPhase'] then
    ...
    game.print('[font=default-large-bold]Picking phase started, captains will pick their team members[/font]',
               Color.cyan)
  end
...
if true then
  commands.add_command('cpt-test-func', 'Run some test-only code for captains games', function(event)
    ...
    for _, playerName in ipairs({
      ...
      local group_name = ''
      if startswith(playerName, 'eve') then
        group_name = '[cpt_eve]'
      end
      if startswith(playerName, 'greg') then
        group_name = '[cpt_greg]'
      end
    ...
    special['minTotalPlaytimeToPlay'] = 0
		...
		table.insert(global.special_games_variables.captain_mode.captainList, game.player.name)
		table.insert(global.special_games_variables.captain_mode.captainList, game.player.name)
  end)
end
...
local function on_gui_click(event)
  ...
  elseif element.name == 'captain_player_want_to_be_captain' then
    if not special['initialPickingPhaseStarted'] and not isStringInTable(special['captainList'], player.name) and
        isStringInTable(special['listPlayers'], player.name) then
  ...
  elseif element.name == 'referee_force_picking_to_stop' then
    if special['pickingPhase'] then
      ...
      game.print('[font=default-large-bold]Referee ' .. player.name .. ' has forced the picking phase to stop[/font]',
                 Color.cyan)
    end
  ...
  elseif element.name == 'captain_toggle_throw_science' then
    if special['captainList'][2] == player.name then
      ...
      game.forces['south'].print('Can anyone throw science in your team ? ' ..
                                     tostring(special['southEnabledScienceThrow']), { r = 1, g = 1, b = 0 })
    else
      special['northEnabledScienceThrow'] = not special['northEnabledScienceThrow']
      game.forces['north'].print('Can anyone throw science in your team ? ' ..
                                     tostring(special['northEnabledScienceThrow']), { r = 1, g = 1, b = 0 })
    end
    Public.draw_captain_manager_gui(player)
  ...
  elseif string.find(element.name, 'captain_bucket_player_') == 1 then
    ...
    if not isStringInTable(playerBucket, player_to_move) then
      return
    end
  ...
  elseif element.name == 'captain_referee_toggle_button' then
    Public.toggle_captain_referee_gui(player)
	elseif element.name == 'captain_tournament_button' then
  ...
  elseif element.name == 'captain_add_someone_to_throw_trustlist' then
    local playerNameUpdateText = get_dropdown_value(
                                     player.gui.screen['captain_manager_gui']['captain_manager_root_table']['captain_add_trustlist_playerlist'])
  ...
  elseif element.name == 'captain_remove_someone_to_throw_trustlist' then
    local playerNameUpdateText = get_dropdown_value(
                                     player.gui.screen['captain_manager_gui']['captain_manager_root_table']['captain_remove_trustlist_playerlist'])
    if playerNameUpdateText and playerNameUpdateText ~= '' then
      local tableToUpdate = special['northThrowPlayersListAllowed']
      local forceForPrint = 'north'
      if player.name == special['captainList'][2] then
...
local function changeCaptain(cmd, isItForNorth)
  ...
  if global.special_games_variables['captain_mode']['prepaPhase'] then
    return playerOfCommand.print('This command is only allowed when prepa phase of event is over, wait for it to start',
                                 Color.red)
  end
      ...
      if isItForNorth then
        if victim.force.name ~= 'north' then
          return playerOfCommand.print(
                     'You cant elect a player as a captain if he is not in the team of the captain ! What are you even doing !',
                     Color.red)
        end
        game.print(
...
commands.add_command('captainDisablePicking', 'Convert to a normal game, disable captain event and tournament mode',
  ...
  global.active_special_games['captain_mode'] = false
  global.tournament_mode = false
  game.print('Players are now free to join whatever team they want (tournament mode disabled), choice made by ' ..
                 playerOfCommand.name, Color.green)
  clear_gui_captain_mode()
end)
