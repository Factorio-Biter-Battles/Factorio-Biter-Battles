...
local function pick_player_generator(player, tableBeingLooped, name, caption, button1Text, button1Name, location)
  ...
  do -- pick table
    ...
    local inner_frame = flow.add { type = 'frame', name = 'inner_frame', style = 'a_inner_paddingless_frame', direction = 'vertical' }
      ...
      for _, pl in pairs(tableBeingLooped) do
        if button1Text ~= nil then
          ...
          if global.total_time_online_players[playerIterated.name] then
            playtimePlayer = PlayerList.get_formatted_playtime_from_ticks(global.total_time_online_players[playerIterated.name])
          end
          if starts_with(playerIterated.tag, ComfyPanelGroup.COMFY_PANEL_CAPTAINS_GROUP_PLAYER_TAG_PREFIX) then
                ...
                if plOfGroup ~= pl then
                  ...
                  if groupNameOtherPlayer ~= '' and groupName == groupNameOtherPlayer then
                    ...
                    if global.total_time_online_players[nameOtherPlayer] then
                      playtimePlayer = PlayerList.get_formatted_playtime_from_ticks(global.total_time_online_players[nameOtherPlayer])
                    end
                    make_table_row(t, button1Name, button1Text, plOfGroup, groupName, playtimePlayer)
                  end
                end
              end
            end
...
local function generate_generic_rendering_captain()
  ...
  y = y + 2
  render_text('captainLineTwo', '-Use of /nth /sth /north-chat /south-chat /s /shout by spectator can be punished (warn-tempban event)', { -65, y }, Color.captain_versus_float, 3, 'heading-1')
  y = y + 4
  render_text('captainLineThree', 'Notes: ', { -65, y }, { 1, 1, 1, 1 }, 2.5, 'heading-1')
  y = y + 2
  render_text('captainLineFour', '-Chat of spectator can only be seen by spectators for players', { -65, y }, { 1, 1, 1, 1 }, 2.5, 'heading-1')
  y = y + 2
  render_text('captainLineSix', '-Teams are locked, if you want to play, click "Join captain event" in your Tournament menu', { -65, y }, { 1, 1, 1, 1 }, 2.5, 'heading-1')
  y = y + 2
  render_text('captainLineSeven', '-We are using discord bb for comms (not required), feel free to join to listen, even if no mic', { -65, y }, { 1, 1, 1, 1 }, 2.5, 'heading-1')
  y = y + 2
  render_text('captainLineEight', '-If you are not playing, you can listen to any team, but your mic must be off', { -65, y }, { 1, 1, 1, 1 }, 2.5, 'heading-1')
  y = y + 2
  render_text('captainLineNine', '-No sign up required, anyone can play the event!', { -65, y }, { 1, 1, 1, 1 }, 2.5, 'heading-1')
  y = y + 2
end
...
local function is_player_in_group_system(playerName)
  ...
  if global.special_games_variables.captain_mode.captainGroupAllowed then
    ...
    if playerChecked and playerChecked.tag ~= '' and
        starts_with(playerChecked.tag, ComfyPanelGroup.COMFY_PANEL_CAPTAINS_GROUP_PLAYER_TAG_PREFIX) then
      return true
    end
  end
  return false
...
local function generate_captain_mode(refereeName, autoTrust, captainKick, specialEnabled)
  ...
  rendering.clear()
  render_text('captainLineTen', 'Special Captain\'s tournament mode enabled', { 0, -16 }, { 1, 0, 0, 1 }, 5, 'heading-1')
  render_text('captainLineEleven', 'team xx vs team yy. Referee: ' .. refereeName .. '. Teams on VC', { 0, 10 }, Color.captain_versus_float, 1.5, 'heading-1')
...
local function start_captain_event()
  ...
  rendering.clear()
  render_text('captainLineSeventeen', 'Special Captain\'s tournament mode enabled', { 0, -16 }, { 1, 0, 0, 1 }, 5, 'heading-1')
  generate_vs_text_rendering()
  generate_generic_rendering_captain()
  render_text('captainLineEighteen', 'Want to play? Click "Join captain event" in your Tournament menu!', { 0, -9 }, { 1, 1, 1, 1 }, 3, 'heading-1')
...
local function on_gui_switch_state_changed(event)
  ...
  elseif name == 'comfy_panel_tournament_gui' then
    ...
    global.captain_ui[player.name].captain_tournament_gui = not global.captain_ui[player.name].captain_tournament_gui
...
local function on_gui_click(event)
  ...
  elseif name == 'captain_player_want_to_be_captain' then
    if not special.initialPickingPhaseStarted and not table_contains(special.captainList, player.name) and table_contains(special.listPlayers, player.name) then
...
function Public.draw_captain_join_info(player, main_frame)
  ...
  gui_style(label, { single_line = false, font_color = { 255, 255, 255 }, horizontally_squashable = true, horizontally_stretchable = true })
end
...
function Public.draw_captain_player_gui(player, main_frame)
  ...
  do -- Title
    ...
    gui_style(title, { horizontally_stretchable = true, vertically_stretchable = true, vertical_align = 'center', horizontal_align = 'center' })

    Gui.add_pusher(title)

    button = title.add { type = 'sprite-button', sprite = 'utility/side_menu_achievements_icon', style = 'transparent_slot' }
  ...
  do -- Preparation flow
    ...
    label = prepa_flow.add { type = 'label', name = 'want_to_play_players_list', style = 'label_with_left_padding', caption = 'want_to_play_players_list' }
    gui_style(label, { single_line = false })

    label = prepa_flow.add { type = 'label', name = 'captain_volunteers_list', style = 'label_with_left_padding', caption = 'captain_volunteers_list' }
    
    label = prepa_flow.add { type = 'label', name = 'remaining_players_list', style = 'label_with_left_padding', caption = 'remaining_players_list' }
    gui_style(label, { single_line = false })
  end
  ...
  do -- Status
    label = main_frame.add { type = 'label', name = 'status_label', caption = 'status_label', style = 'label_with_left_padding' }
    gui_style(label, { single_line = false })
  end
  ...
  do -- PLayer table 
    ...
    button = title.add { type = 'sprite-button', sprite = 'utility/slot_icon_inserter_hand', style = 'transparent_slot' }
    button.ignored_by_interaction = true
    gui_style(button, { size = 40 })
...
function Public.draw_captain_tournament_gui(player)
  ...
  local sp = inner_frame
	...
	local function add_shortcut_selection_row(parent, p)
		local frame = parent.add { type = 'frame', name = p.name, style = 'shortcut_selection_row', direction = 'horizontal' }
...
function Public.draw_captain_tournament_frame(player)
  ...
  local frame = main_frame.add { type = 'frame', name = 'frame', style = 'inside_deep_frame', direction = 'vertical' }
  ...
  local function add_frame_row(parent, p)
    ...
    do
      local icon = head.add { type = 'sprite-button', name = 'icon', sprite = p.sprite, style = 'transparent_slot' }
    ...
    local body = inner_flow.add { type = 'frame', name = 'frame', style = 'inside_deep_frame', direction = 'vertical' }
    gui_style(body, { top_padding = 5, bottom_padding = 5, left_padding = 10, right_padding = 10, horizontally_stretchable = true })
...
function Public.update_captain_player_gui(player, frame)
  ...
  do -- Status & Join buttons
    ...
    if global.chosen_team[player.name] then
      insert(status_strings, 'On team ' .. global.chosen_team[player.name] .. ': ' .. Functions.team_name_with_color(global.chosen_team[player.name]))
    elseif special.kickedPlayers[player.name] then
      insert(status_strings, 'You were kicked from a team, talk to the Referee about joining if you want to play.')
    ...
    elseif special.pickingPhase then
      insert(status_strings, 'A picking phase is currently active, wait until it is done before you can indicate that you want to play.')
    end
    ...
    if not global.chosen_team[player.name] and not special.pickingPhase and not special.kickedPlayers[player.name] then
      ...
      if special.prepaPhase and not special.initialPickingPhaseStarted then
        if special.captainGroupAllowed then
          insert(status_strings, string_format('Groups of players: ENABLED, group name must start with "%s"', ComfyPanelGroup.COMFY_PANEL_CAPTAINS_GROUP_PREFIX))
  ...
  do -- Player table
    ...
    if #sorted_players > 0 then
      ...
      for _, player_name in pairs(sorted_players) do
        ...
        label = tab.add { type = 'label', caption = Functions.team_name_with_color(info.force), style = 'valid_mod_label' }
        label = tab.add { type = 'label', caption = info.picked_at and Functions.format_ticks_as_time(info.picked_at) or '', style = 'valid_mod_label' }
        label = tab.add {
          type = 'label',
          caption = string_format('%s (%d%%)', Functions.format_ticks_as_time(info.playtime), 100 * playtime_frac),
...
commands.add_command('captainDisablePicking', 'Convert to a normal game, disable captain event and tournament mode', function(cmd)
  if not cmd.player_index then
    return
  end
  local playerOfCommand = cpt_get_player(cmd.player_index)
