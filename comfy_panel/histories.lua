...
local comparators = {
    ['name_asc'] = function(a, b)
        return a.player_name:lower() < b.player_name:lower()
    end,
    ...
    ['event_asc'] = function(a, b)
        return a.event:lower() < b.event:lower()
    end,
    ['event_desc'] = function(a, b)
        return a.event:lower() > b.event:lower()
    end,
}
...
local function draw_events(player, frame)
    ...
    local header_modifier = {
        ...
        ['event_asc'] = function(h)
            h[3] = symbol_asc .. h[3]
        end,
        ['event_desc'] = function(h)
            h[3] = symbol_desc .. h[3]
        end,
    }
    ...
    for k, v in pairs(temp) do
        ...
        frame.history_scroll.history_table.add({ type = 'label', caption = formatted_time }).style.width =
            column_widths[1]
        frame.history_scroll.history_table.add({ type = 'label', caption = v.player_name }).style.width =
            column_widths[2]
        frame.history_scroll.history_table.add({ type = 'label', caption = v.event }).style.width = column_widths[3]
        frame.history_scroll.history_table.add({
            type = 'label',
            name = 'coords_' .. k,
            caption = v.position.x .. ' , ' .. v.position.y,
        }).style.width =
            column_widths[4]
    end
end
...
local create_histories_panel = function(player, frame)
    ...
    filter_table.add({
        type = 'drop-down',
        name = 'history_select',
        items = histories_names,
        selected_index = this.selected_history_index[player.name],
    })
    ...
    flow.add({
        ...
        tooltip = 'Click this button and then ping on map to filter history',
    })
