...
local function generate_power_feed(flasks_number, flask_type, min_charge, capacity)
    ...
    for _, v in ipairs(silos) do
        ...
        if v.force.name == "north" then
            power_pole = surface.create_entity {
                name = "medium-electric-pole",
                position = {v.position.x, v.position.y + 5},
                force = v.force
            }
        ...
        elseif v.force.name == "south" then
            power_pole = surface.create_entity {
                name = "medium-electric-pole",
                position = {v.position.x, v.position.y - 6},
                force = v.force
            }
        ...
        for i, b in ipairs({-4, -2, 1, 3}) do
            local energy_interface = surface.create_entity {
                ...
                position = {power_pole.position.x + b, power_pole.position.y},
                force = v.force
            }
...
function Public.feed_energy(player)
    ...
    local accumulators = surface.find_entities_filtered {
        name = {"accumulator", "electric-energy-interface"},
        force = player.force,
        to_be_deconstructed = false
    }
    for _, i in ipairs(accumulators) do
        if i.electric_network_id == main_pole.electric_network_id then total_charge = total_charge + i.energy end
    end
    ...
    -- clearing accus
    for _, i in ipairs(accumulators) do i.energy = 0 end
...
Event.on_init(function() for k, v in pairs(valid_special_games) do global.active_special_games[k] = "false" end end)
