...
local function get_or_create_dummy_player(playerName)
  ...
  return game.get_player(playerName) or {
...
function Public.get_colored_player_list(player_list)
	...
	for _, player in pairs(player_list) do
		table_insert(players_with_colors, string.format('[color=%.2f,%.2f,%.2f]%s[/color]', player.color.r * 0.6 + 0.4, player.color.g * 0.6 + 0.4, player.color.b * 0.6 + 0.4, player.name))
	end
