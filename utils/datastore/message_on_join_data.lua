...
local fetch = Token.register(function(data)
    ...
    if type(value) == 'table' then
        game.print('>> ' .. player.name .. ' << ' .. value.msg, { color = value.color })
    end
end)
...
commands.add_command('save-message', 'Sets your custom join message.', function(cmd)
    ...
    if param then
        if param == '' or param == 'Name' then
            return player.print('You did not specify a message.', { color = Color.warning })
        end
        if string.len(param) > 64 then
            return player.print(
                'Message is too long. 64 characters maximum.',
                { color = { r = 0.90, g = 0.0, b = 0.0 } }
            )
        end
        set_data(message_dataset, player.name, { msg = param, color = player.color })
        player.print('You message has been saved.', { color = Color.success })
    else
        player.print('You did not specify a message.', { color = Color.warning })
    end
end)
...
commands.add_command('remove-message', 'Removes your custom join message.', function()
    ...
    player.print('Your message has been removed.', { color = Color.success })
end)
