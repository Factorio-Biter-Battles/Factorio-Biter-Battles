...
local function draw_suspend_gui(player)
  ...
  local frame = Gui.add_top_element(player, { type = 'frame', name = 'suspend_frame', style = 'finished_game_subheader_frame' })
  ...
  local line = f.add({ type = 'line', direction = 'vertical' })
	...
	do -- buttons
		...
		local l = t.add { type = 'label', caption = {'gui.suspend_caption', global.suspend_target_info.suspendee_player_name, global.suspend_time_left } }
		gui_style(l, {
...
local suspend_token = Token.register(function()
  ...
  if suspend_info ~= nil then
    ...
    if total_votes > 0 then
      ...
      if result >= 75 and total_votes > 1 then
        ...
        Server.to_banned_embed(table.concat {
          suspend_info.suspendee_player_name .. ' was suspended ( ' .. result .. ' %)' .. ', vote started by ' ..
    ...
    if total_votes == 1 and result == 100 then
      game.print('Vote to suspend ' .. suspend_info.suspendee_player_name .. ' has failed because only 1 player voted, need at least 2 votes')
      Server.to_banned_embed(table.concat {
      ...
      Server.to_banned_embed(table.concat {
        suspend_info.suspendee_player_name .. ' was not suspended and vote failed ( ' .. result .. ' %)' ..
            ', vote started by ' .. suspend_info.suspender_player_name,
      })
    end
    global.suspend_target_info = nil
  end
...
commands.add_command('suspend', 'Force a player to stay in spectator for 10 minutes : /suspend playerName', function(cmd)
  suspend_player(cmd);
end)
